 ...
 * <p>
 * JWT 인증을 위한 무상태/web security/Jwt filter 를 등록하였습니다.
 *
 *
 * <p><b>빈 관리:</b><br>
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, JwtUtils jwtUtils) throws Exception {
        http
                ...
                .authorizeHttpRequests((auth) -> {
                    AccessURL.PUBLIC.getUrls().forEach(url -> auth.requestMatchers(url).permitAll());
                    auth.anyRequest().authenticated();
                })

                .addFilterBefore(new JwtAuthenticationFilter(jwtUtils, objectMapper),
